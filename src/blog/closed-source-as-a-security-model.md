...
What is actually happening is that frontier models are increasing the available search space for vulnerabilities, and automating the task of running it. And the actual question is the practical tradeoff between **attacker cost** and **defender cost**. If open code meaningfully helps attackers automate finding exploits faster than your team can remediate them, then limiting exposure may buy time; but if the dominant risk comes from implementation flaws, weak auth, missing controls, or poor operational hygiene, then closing the source does little while also reducing external review.
...
The calculus is simple: discovery + execution cost + remediation cost on the defensive side vs discovery and execution on the attacking side.
...
Is **Y + Z > X**? If you assume that more eyes means that not only are you funding the discovery, then the answer is likely yes. Then there is the issue of time that the vulnerability exists, which makes the probability of finding it higher.
